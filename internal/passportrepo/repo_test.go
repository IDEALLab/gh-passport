...
func (runner repositoryRunner) Run(_ context.Context, _ string, _ string, args ...string) ([]byte, error) {
	...
	case "remote get-url upstream":
		return []byte("https://github.com/IDEALLab/passport-exercises.git\n"), nil
...
func TestLoadPassportRequiresAssignedAssessmentBranch(t *testing.T) {
	...
	valid := `{"schema_version":2,"curriculum_version":"1.2.0","github_user":"Student-1","source_repository":"IDEALLab/passport-exercises","fork_repository":"Student-1/passport-exercises","assessment_branch":"onboarding/student-1","platform":"linux","responsibilities":[],"missions":["core-orientation"]}`
	...
	invalid := `{"schema_version":2,"curriculum_version":"1.2.0","github_user":"Student-1","source_repository":"IDEALLab/passport-exercises","fork_repository":"Student-1/passport-exercises","assessment_branch":"main","platform":"linux","responsibilities":[],"missions":["core-orientation"]}`
...
func TestLoadPassportRejectsUsernameThatCannotOwnAssignedBranch(t *testing.T) {
	directory := t.TempDir()
	path := filepath.Join(directory, "passport.json")
	invalid := `{"schema_version":2,"curriculum_version":"1.2.0","github_user":"student_name","source_repository":"IDEALLab/passport-exercises","fork_repository":"student_name/passport-exercises","assessment_branch":"onboarding/student_name","platform":"linux","responsibilities":[],"missions":["core-orientation"]}`
	if err := os.WriteFile(path, []byte(invalid), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestFindAcceptsOnlyTheAssignedAssessmentBranch(t *testing.T) {
	...
	passport := `{"schema_version":2,"curriculum_version":"1.2.0","github_user":"student","source_repository":"IDEALLab/passport-exercises","fork_repository":"student/passport-exercises","assessment_branch":"onboarding/student","platform":"linux","responsibilities":[],"missions":["core-orientation"]}`
